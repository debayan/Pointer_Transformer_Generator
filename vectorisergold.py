...
class Vectoriser():
    def __init__(self, proppath):
       ...
       for item in json.loads(open(proppath).read()):
           self.props[item['id']] = item['description']
...
#        print("relations: ",rels)
        ...
        candidatevectors = [[0,0,0,0]+embedding+200*[0.0] for embedding in questionembeddings]#list(map(lambda x: sum(x)/len(x), zip(*questionembeddings)))
...
#            print("labelembedding: ",labelembedding)
            candidatevectors.append([1,1,1,1]+labelembedding+entityembedding) #1,1,1,1 marks an entity, since 504 has many factors needed later for attention blocks
...
#            print("labelembedding: ",labelembedding)
            candidatevectors.append([2,2,2,2]+labelembedding+relembedding)
