...
for idx,item in enumerate(d):
    ...
    print(str(item['uid']))
    print(item['question'])
    target_ = item['querytemptar']
    answer_ = item['querytempans']
    ents = item['goldents']
    rels = item['goldrels']
    print("ents: ",ents)
    print("rels: ",rels)
    ...
    if resulttarget == resultanswer:
        ...
        print("nomatch")
        nem += 1
        querywrong.append({'querytemptar':target_, 'querytempans': answer_,'id':str(item['uid']),'question':item['question'],'ents':ents,'rels':rels,'resulttarget':resulttarget,'resultanswer':resultanswer})
