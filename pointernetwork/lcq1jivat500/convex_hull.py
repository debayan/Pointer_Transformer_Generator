...
class ConvexHull(object):
  def __init__(self, forward_only):
    ...
    with self.testgraph.as_default():
      config = tf.ConfigProto()
      config.gpu_options.allow_growth = True
      config.operation_timeout_in_ms=600
      self.testsess = tf.Session(config=config)
    self.build_model()
    data = open(FLAGS.data_path).readlines()
    size = len(data)
    self.read_data(data[:int(0.9*size)])
    self.test_read_data(data[int(0.9*size):])
    self.id2word = json.loads(open('id2word_lcq13253_jivat_w_'+FLAGS.modelnum+'.txt').read())
    ...
    while True:
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
          ...
          f1 = self.eval()
          #f1 = kbcorrector.f1eval('predict_logs/lcq3253_500_shuffled_lr0.01_modelout-fold'+FLAGS.modelnum+'.json',FLAGS.modelnum)
          print("f1: ",f1)
          if f1 and f1 > bestf1:
