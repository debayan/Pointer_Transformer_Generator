...
def train_step(features, labels, params, model, optimizer, loss_object, train_loss_metric, batchcount, testbatcher):
        ...
        with tf.GradientTape() as tape:
                ...
                #print("decinput: ",labels["dec_input"][0])
                #print("dectarget: ",labels["dec_target"][0])
            ...
            for testidx,testbatch in enumerate(testbatcher):
                ...
                for answer,target,uid,question,oov,entrels in zip(output,testlabels["dec_target"],testfeatures["uid"],testfeatures["question"],testfeatures['question_oovs'],testfeatures['entrels']):
                    try:
                        ...
                        answer__ = [] 
                        for answerid in list(answer[1:].numpy()):
                            if answerid == 3 or answerid == 1:
                                break
