...
def train_step(features, labels, params, model, optimizer, loss_object, train_loss_metric, batchcount, testbatcher, propembeddingtensor):
        ...
        with tf.GradientTape() as tape:
                ...
                output, attn_weights = model(questions,propembeddingtensor,features["old_enc_input"],features["extended_enc_input"], features["max_oov_len"], labels["dec_input"], training=params["training"], 
...
def train_model(model, batcher, testbatcher, params, ckpt, ckpt_manager, propembeddings):
        ...
        propembeddingslist = []
        for item in propembeddings:
            embed = [float(x) for x in item['embedding']]
            propembeddingslist.append(embed)
        x_ = np.array(propembeddingslist)
        propembeddingtensor = tf.convert_to_tensor(np.broadcast_to(x_, (params["batch_size"],x_.shape[0],x_.shape[1] )), dtype=tf.float32)
        ...
        try:
                ...
                while int(ckpt.step) < params["max_steps"]:
                        ...
                        for idx,batch in enumerate(batcher):
                                t0 = time.time()
                                valfuzz,qcount = train_step(batch[0], batch[1], params, model, optimizer, loss_object, train_loss_metric, idx, testbatcher,propembeddingtensor)
