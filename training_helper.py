...
def train_step(features, labels, params, model, optimizer, loss_object, train_loss_metric, batchcount, testbatcher):
        ...
        with tf.GradientTape() as tape:
                ...
                output, attn_weights, p_gens = model(questions,features["enc_input"],features["extended_enc_input"], features["max_oov_len"], labels["dec_input"], training=params["training"], 
            ...
            for testidx,testbatch in enumerate(testbatcher):
                try:
                    ...
                    for i in range(params["max_dec_len"]):
                        ...
                        predictions, test_attn_weights, p_gens = model(testfeatures["question"],testfeatures["enc_input"],testfeatures["extended_enc_input"], testfeatures["max_oov_len"], output, training=False, enc_padding_mask=test_enc_padding_mask, look_ahead_mask=test_combined_mask,dec_padding_mask=test_dec_padding_mask)
